...
def apply_nlp_operations(df):
    # !!!!!!!!!!!!!!!!!NLP_BANK!!!!!!!!!!!!!!!!!!
    df["3Year_Payout Ratio"] = df["BANK_DIVIDENDS_IsDividendCoveredIn3Years_Desc"].apply(extract_percentage)
    ...
    # !!!!!!!!!!!!!!!!!NLP_DIVIDENDS!!!!!!!!!!!!!!!!!!
    df["Cash Payout Ratio (%)"] = df["DIVIDENDS_IsDividendCoveredByFreeCashFlow_Desc"].apply(extract_cash_payout_ratio)
    df["Payout Ratio (%)"] = df["DIVIDENDS_IsDividendCovered_Desc"].apply(extract_payout_ratio)
    df[["Dividend Yield %", "Bottom 25% Market Dividend Yield %"]] = df["DIVIDENDS_IsDividendSignificant_Desc"].apply(lambda x: pd.Series(extract_dividend_yields(x)))
    ...
    # !!!!!!!!!!!!!!!!!NLP_FUTURE!!!!!!!!!!!!!!!!!!
    df[["3-Year Annual Earnings Growth Forecast %", "3-Year Annual Market Earnings Growth Forecast %"]] = df["FUTURE_IsExpectedAnnualProfitGrowthAboveMarket_Desc"].apply(lambda x: pd.Series(extract_growth_forecasts(x)))
    df["Savings Rate %"] = df["FUTURE_IsExpectedProfitGrowthAboveRiskFreeRate_Desc"].apply(extract_savings_rate)
    df['Annual Revenue Growth %'], df['Annual Market Revenue Growth %'] = zip(*df['FUTURE_IsExpectedRevenueGrowthAboveMarket_Desc'].apply(extract_percentages))
    ...
    df["ROE 3Y Forecast %"] = df["FUTURE_IsFruitfulFutureIntro_Desc"].apply(extract_roe)
    ...
    # Create a regex pattern that matches any of the given prefixes
...
for base_directory in base_directories:
    ...
    for root, dirs, files in os.walk(base_directory):
        for file in files:
            # Filter files starting with "Transposed" and ending with ".csv"
            if "transposed" in file.lower() and file.endswith(".csv"):
                ...
                try:
                    ...
                    # Apply all NLP operations
                    df = apply_nlp_operations(df)
                    # Generate the new file name with 'NLP_' prefix
                    new_file_name = f"NLP_{file}"
                    # Save the processed DataFrame to the same folder with the new name
